...
A python script that takes a text file containing a list of commands used to configure routers and switches. The script then strips away all text leading up to the hash symbol that prevent the networking device from accepting the command. These commands then can easily be copied onto a terminal or CLI for usage.
